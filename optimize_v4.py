...
def run_fast(px, scores, top, cost_bps, vol_m=None, risk_weight=False, vol_target=None):
    ...
    if vol_target:
        ...
        for s in range(1,T):
            r0=day[np.searchsorted(day,me[s],side="right")]
            if r0>=day[0]:
                look=ug.loc[:pd.Timestamp(r0)].iloc[-(61):-1].dropna()
                if len(look)>=21 and look.std(ddof=1)>0:
                    scale[day==r0]=min(1.0,vol_target/(look.std(ddof=1)*np.sqrt(DAYS)))
        scdf=pd.DataFrame(np.tile(scale[:,None],(1,M)),index=px.index,columns=cols)
        scdf=scdf.where(Wdf.abs().sum(axis=1)>0,1.0)
        scale_ser=pd.Series(scale,index=px.index)
        # assign scale by month block
        for s in range(1,T):
            lo=day[np.searchsorted(day,me[s],side="right")]
            up=me[s+1] if s+1<T else day[-1]
            sel=(day>=lo)&(day<=up)
            if np.any(sel) and scale_ser.iloc[np.where(sel)[0][0]]>0:
                scale_ser.iloc[sel]=scale_ser.iloc[np.where(sel)[0][0]]
        Wdf=Wdf.mul(scale_ser,axis=0)
    g=(Wdf*dr).sum(axis=1).fillna(0.0)
    cost=pd.Series(0.0,index=g.index); prev=np.zeros(M)
    for s in range(1,T):
        lo=day[np.searchsorted(day,me[s],side="right")]
        if lo<=day[-1]:
            i=np.searchsorted(day,lo)
            wn=Wmat[slot[i]]
            to=np.abs(wn-prev).sum()/2.0
            cost.iloc[i]=to*cost_bps/10000.0
            prev=wn.copy()
...
for p in [3,6,9,12]:
    for k in [0,1]:
        ...
        for top in [10,20]:
            ...
            isre=stats(ret[(ret.index>=pd.Timestamp('2017-02-01'))&(ret.index<=pd.Timestamp('2021-12-31'))],
                      nav[(ret.index>=pd.Timestamp('2017-02-01'))&(ret.index<=pd.Timestamp('2021-12-31'))])
            oore=stats(ret[(ret.index>=pd.Timestamp('2022-01-01'))],nav[(ret.index>=pd.Timestamp('2022-01-01'))])
            fre=stats(ret,nav)
