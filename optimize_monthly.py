# -*- coding: utf-8 -*-
"""月频策略优化扫描：在月频 6m-skip1 top10 基础上试多种增强
...
def run(variant, top, maxk=3, vol_target=None, spy_ma=False):
    rb = [d for d in monthly_last_days() if d > 189 and d+1 < len(idx)]
...
variants = [
    ("base（月频6m-skip1 top10）", "base", 10, None, False),
...
out=OUT/"monthly_optimize_scan.csv"; df.to_csv(out, index=False, encoding="utf-8-sig")
