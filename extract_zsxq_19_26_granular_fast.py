...
def call_llm(text, key, base, model, timeout=90):
    ...
    for attempt in range(2):
        try:
            ...
            return arr if isinstance(arr, list) else [], model
...
def call_llm_with_fallback(text, key, base, primary_model):
    ...
    for model in models:
        if model in EXHAUSTED_MODELS:
            continue
        result, used = call_llm(text, key, base, model)
...
def prefilter(text):
    ...
    if re.search(r"\b[A-Z]{1,6}\.US\b", text): return True
    if re.search(r"\b(NVDA|AMD|INTC|MU|WDC|STX|MRVL|AMAT|GLW|FLEX|DELL|NBIS|ARM|LITE|AVGO|SNPS|ADI|KLAC|LRCX|MCHP|ON|NXPI|TXN|QCOM|SMCI|META|AAPL|MSFT|GOOGL|AMZN|TSLA|LLY|MRNA|MRK|COIN|PLTR|TSM|UBER|NFLX|RBLX)\b", text): return True
    if any(k in text for k in CN2T if len(k) >= 2): return True
    inst = ["摩根大通","大摩","高盛","瑞银","野村","美银","巴克莱","摩根士丹利","花旗","杰富瑞","Raymond James","德意志","汇丰","法兴","瑞信","伯恩斯坦"]
    tech_kw = ["AI", "GPU", "芯片", "半导体", "算力", "光模块", "数据中心", "云计算", "大模型", "美股"]
    if any(x in text for x in inst) and any(x in text for x in tech_kw): return True
    return False

def process_one(args):
    idx, it, key, base, model, cache_path = args
    text = it.get("text", "")
    date = re.search(r"(\d{4}-\d{2}-\d{2})", it.get("create_time", "") or "").group(1) if re.search(r"(\d{4}-\d{2}-\d{2})", it.get("create_time", "") or "") else ""
    sig = f"{date}|{text[:80]}"

    events, used_model = call_llm_with_fallback(text, key, base, model)

    cache_obj = {"sig": sig, "create_time": it.get("create_time"), "events": events, "model": used_model}
    line = json.dumps(cache_obj, ensure_ascii=False) + "\n"
    if _lock:
        with _lock:
    ...
    rows = []
    for ev in events:
        if not ev.get("is_us_stock"): continue
        ticker = ev.get("ticker", "").upper().strip()
        if not ticker or ticker not in US_TICKERS: continue
        ...
        rows.append({
            "event_time": event_time, "date": date, "ticker": ticker,
            ...
            "source_text": text[:500], "author": it.get("author", ""),
...
def main():
    ...
    ap.add_argument("--model", default=os.environ.get("LLM_MODEL", "deepseek-v4-flash-202605"))
    a = ap.parse_args()

    items = json.loads((OUT / "zsxq_group_48418411254128_web.json").read_text(encoding="utf-8"))
    ...
    print(f"[llm] primary={a.model} @ {base}, workers={a.workers}")
    ...
    for idx, it in filtered:
        ...
        sig = f"{date}|{text[:80]}"
        if sig not in cache:
            todo.append((idx, it, key, base, a.model, str(cache_path)))
    print(f"[todo] {len(todo)} topics to process")

    processed = len(cache)
    total_events = 0
    with ThreadPoolExecutor(max_workers=a.workers, initializer=init_worker) as ex:
        futures = {ex.submit(process_one, arg): arg[0] for arg in todo}
        for fut in as_completed(futures):
            idx, n_events = fut.result()
            processed += 1
            total_events += n_events
            if processed % 50 == 0:
                print(f"[progress] {processed}/{len(filtered)} -> total events ~{total_events}")
    ...
    if cache_path.exists():
        for line in cache_path.read_text(encoding="utf-8").splitlines():
            ...
            try:
                obj = json.loads(line)
                it = {"text": "", "create_time": obj.get("create_time", ""), "author": ""}
                # 找到原始文本（简化：从 sig 中恢复不了全文，只保存 events）
                for ev in obj.get("events", []):
                    if not ev.get("is_us_stock"): continue
                    ticker = ev.get("ticker", "").upper().strip()
                    if not ticker or ticker not in US_TICKERS: continue
                    event_time = parse_time(obj.get("create_time"))
                    if not event_time: continue
                    sentiment = float(ev.get("sentiment_score", 0))
                    confidence = float(ev.get("confidence", 0))
                    all_rows.append({
                        "event_time": event_time,
                        "date": obj.get("create_time", "")[:10] if obj.get("create_time") else "",
                        "ticker": ticker,
                        "text_type": ev.get("text_type", "noise"),
                        "materiality_tier": ev.get("materiality_tier", "tier_3_macro_industry"),
                        "sentiment_score": sentiment, "confidence": confidence,
    ...
    print(f"[done] processed {processed}, total events {len(all_rows)}")
